# Script PowerShell simple et fonctionnel pour backup de structure MySQL
param(
    [string]$Commentaire = "",
    [string]$OutputPath = "Backups"
)

Write-Host "=== Backup simple et fonctionnel de la base de données DiversityPub ===" -ForegroundColor Green
...
# Créer le contenu SQL en utilisant Add-Content
$header = @"
/* =============================================
...
try {
    $header | Out-File -FilePath $backupFilePath -Encoding UTF8
    ...
    $metadata = @{
        ...
        Method = "Script PowerShell simple et fonctionnel"
    }
...
Write-Host "=== Backup simple et fonctionnel terminé ===" -ForegroundColor Green
